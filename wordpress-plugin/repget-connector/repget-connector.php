...
/**
 * Carries a key from RepGet into the form, so it never has to be copied.
 *
 * RepGet links here with ?repget_key=..., and the field arrives filled in.
 * The customer presses Save and connect; that submission is nonce-checked
 * like any other, which is what keeps this safe.
 *
 * IT DOES NOT CONNECT BY ITSELF, and that is deliberate rather than a
 * shortcut not taken. Acting on a GET would be a textbook CSRF: an image tag
 * on any page could repoint a logged-in administrator's site at somebody
 * else's RepGet account. The obvious fix - a nonce - cannot work here,
 * because a nonce is generated by WordPress and RepGet has no way to produce
 * one. So the link prepares the form and a human presses the button, which
 * removes the step that actually fails (a partial copy-paste) without
 * inventing a hole.
 */
function repget_prefill_from_link() {
    if (!isset($_GET['repget_key']) || !current_user_can('manage_options')) {
        return '';
    }
    return sanitize_text_field(wp_unslash($_GET['repget_key']));
}
...
function repget_settings_page() {
    ...
    /*
      A key handed over by the link RepGet generated. Shown in the field,
      not saved - see repget_prefill_from_link.
    */
    $prefilled = repget_prefill_from_link();
    ...
    /*
      A key from the RepGet link fills the field; otherwise whatever is
      already saved. The link wins so that somebody reconnecting - a new key
      after revoking an old one - sees the NEW key rather than the stale one
      they came here to replace.
    */
    $key = $prefilled !== '' ? $prefilled : repget_key();
